...
public class PropertyStatisticsImproved {
	...
	private static void reCreateTriples(List<String> dbProps,
		...
		try {
			shoudBeIn(dbProps, domainType, rangeType, line, triplesWriter,
					candidateSubjs.get(0).split("\t")[0], candidateObjs.get(0)
							.split("\t")[0]);
		} catch (Exception e) {
		}
	}
	...
	private static void shoudBeIn(List<String> dbProps, String domainType,
			...
			BufferedWriter triplesWriter, String dbpSub, String dbpObj)
			throws IOException {
	...
	private static List<String> filterGeneralMostProperties2(
			List<String> dbpProps) {
		...
		for (int out = 0; out < dbpProps.size(); out++) {
			for (int in = out + 1; in < dbpProps.size(); in++) {
				subProperty = checkPropertySubsumption(dbpProps.get(out),
						dbpProps.get(in));

				if (!toBeRemovedProps.contains(subProperty)
						&& subProperty != null)
	...
	private static String checkPropertySubsumption(String prop1, String prop2) {
		...
		try {
			domainProp1 = SPARQLEndPointQueryAPI
					.queryDBPediaEndPoint(
							"select ?dom where {<"
									+ prop1
									+ "> <http://www.w3.org/2000/01/rdf-schema#domain> ?dom}")
		...
		try {
			rangeProp1 = SPARQLEndPointQueryAPI
					.queryDBPediaEndPoint(
							"select ?ran where {<"
									+ prop1
									+ "> <http://www.w3.org/2000/01/rdf-schema#range> ?ran}")
		...
		try {
			domainProp2 = SPARQLEndPointQueryAPI
					.queryDBPediaEndPoint(
							"select ?dom where {<"
									+ prop2
									+ "> <http://www.w3.org/2000/01/rdf-schema#domain> ?dom}")
		...
		try {
			rangeProp2 = SPARQLEndPointQueryAPI
					.queryDBPediaEndPoint(
							"select ?ran where {<"
									+ prop2
									+ "> <http://www.w3.org/2000/01/rdf-schema#range> ?ran}")
		...
		try {
			domainProp1 = (domainProp1 != null) ? domainProp1.replaceAll(
					Constants.ONTOLOGY_DBP_NS, "") : "null";

			domainProp2 = (domainProp2 != null) ? domainProp2.replaceAll(
					Constants.ONTOLOGY_DBP_NS, "") : "null";

			rangeProp1 = (rangeProp1 != null) ? rangeProp1.replaceAll(
					Constants.ONTOLOGY_DBP_NS, "") : "null";
			rangeProp2 = (rangeProp2 != null) ? rangeProp2.replaceAll(
					Constants.ONTOLOGY_DBP_NS, "") : "null";

			if (isSuperClass2(domainProp1, domainProp2)
					&& isSuperClass2(rangeProp1, rangeProp2)) {
				log.debug(prop1 + " super property of " + prop2);
				return prop2;
			}

			if (isSuperClass2(domainProp2, domainProp1)
					&& isSuperClass2(rangeProp2, rangeProp1)) {
				log.debug(prop2 + " super property of " + prop1);
				return prop1;
			}
		} catch (Exception e) {
			log.error(domainProp1 + "\t" + domainProp2 + "\t" + rangeProp1
					+ "\t" + rangeProp2);
			e.printStackTrace();
		}
		return null;
		...
		for (Map.Entry<String, Map<String, Map<Pair<String, String>, Long>>> entry : GLOBAL_TRANSCS_MAP
			...
			if (entry.getKey().equals(nellProp)) {
				for (Map.Entry<String, Map<Pair<String, String>, Long>> nellVal : entry
					...
					for (Map.Entry<Pair<String, String>, Long> pairs : nellVal
							.getValue().entrySet()) {
						flag = (isSuperClass(pairs.getKey().getFirst(), domain))
								&& isSuperClass(pairs.getKey().getSecond(),
										range);
	...
	private static boolean isSuperClass2(String generalClass,
			String particularClass) {
