...
public class PropertyStatisticsImproved
{
    ...
    private static void createNewTriples(String filePath) throws IOException
    {
        ...
        BufferedWriter statStriplesWriter = new BufferedWriter(new FileWriter(DISTRIBUTION_NEW_TRIPLES));
        ...
        for (ArrayList<String> line : directPropsFile) {
            ...
            if (line.size() == 3)
                if (FINAL_MAPPINGS.containsKey(nellProp)) {
                    ...
                    reCreateTriples(dbProps, line, triplesWriter, statStriplesWriter);
                }
        }
    ...
    private static void shoudBeIn(List<String> dbProps, String domainType, String rangeType, ArrayList<String> line,
        BufferedWriter triplesWriter, BufferedWriter statStriplesWriter, String dbpSub, String dbpObj)
        throws IOException
    {
        ...
        for (String dbprop : dbProps) {
            ...
            if (isSuperClass3(allowedDomain, domainType) && isSuperClass3(allowedRange, rangeType)) {
                triplesWriter.write(line.get(0) + "\t" + line.get(1) + "\t" + line.get(2) + "\t"
                    + Constants.DBPEDIA_INSTANCE_NS + Utilities.utf8ToCharacter(dbpSub) + "\t" + dbprop + "\t"
                    + Constants.DBPEDIA_INSTANCE_NS + Utilities.utf8ToCharacter(dbpObj) + "\n");
                triplesWriter.flush();

                statStriplesWriter.write(line.get(1) + "\t" + domainType + "\t" + allowedDomain + "\t" + dbprop + "\t"
