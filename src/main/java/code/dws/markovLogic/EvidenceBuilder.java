...
public class EvidenceBuilder {
    ...
    public EvidenceBuilder(String[] args) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length < 1)
             ...
             */
            evdBuilder.processTriple(oieFile, OIE.NELL, ",");
        }
    ...
    public void processTriple(File inputFile, OIE oieType, String delimit)
        ...
        BufferedWriter allEvidenceWriterTop1 = new BufferedWriter(new FileWriter(
                Constants.ALL_MLN_EVIDENCE_T1));
        ...
        // Season 2
        // the file where the evidences for the MLN are written out
        BufferedWriter kdeWriter = new BufferedWriter(new FileWriter(
                Constants.KDE_EVIDENCE_ALL));
        ...
        while ((triple = input.readLine()) != null) {
            ...
            if (this.propertyName != null && this.propertyName.length() > 0)
                if (this.propertyName.equals(arrStr[1])) {
                    ...
                    // process them
                    this.createEvidences(arrStr[0], arrStr[1], arrStr[2], arrStr[3],
                            oieType,
                            allEvidenceWriterTop1, allEvidenceWriter, termConceptPairSet,
                            goldEvidenceWriter, kdeWriter);
                }
        }
     ...
     * @throws IOException
     */
    private void createEvidences(String sub, String prop, String obj, String conf, OIE oieType,
            BufferedWriter allEvidenceWriterTop1,
            BufferedWriter allEvidenceWriter, Set<String> termConceptPairSet,
            BufferedWriter goldEvidenceWriter, BufferedWriter kdeWriter)
            throws IOException {
        ...
        allEvidenceWriter.write("propAsstConf(\"NELL#Predicate/" + nellPred
                + "\", \"NELL#Instance/"
                + nellSubPFxd + "\", \"NELL#Instance/" + nellObjPFxd + "\", " + confidence + ")\n");
         ...
         */
        List<String> mappingsSub = createEvidenceForTopKCandidates(allEvidenceWriterTop1,
                allEvidenceWriter, nellSub,
                nellSubPFxd, termConceptPairSet, Constants.DOMAIN);

        /**
         * create top-k evidences for object
         */
        List<String> mappingsObj = createEvidenceForTopKCandidates(allEvidenceWriterTop1,
                allEvidenceWriter, nellObj,
                nellObjPFxd, termConceptPairSet, Constants.RANGE);

//        EstimatorEngine.likelihoodWrapper(mappingsSub, mappingsObj, kdeWriter);

    }

    // private void createGoldEvidence(String sub, String obj, String
    // nellSubPFxd, String nellObjPFxd,
    // BufferedWriter goldEvidenceWriter) throws IOException {
    // Pair<String, String> goldPair = null;
    //
    // if (GS_PAIRS.containsKey(sub + "\t" + obj)) {
    // System.out.println(" IN GS = " + GS_PAIRS.get(sub + "\t" + obj));
    //
    // goldPair = GS_PAIRS.get(sub + "\t" + obj);
    //
    // // Create Gold MLN
    // if (goldPair.getFirst().indexOf("?") == -1)
    // GenericConverter.createGoldSameAsMLN(goldPair.getFirst(), nellSubPFxd,
    // goldEvidenceWriter);
    //
    // // Create Gold MLN
    // if (goldPair.getSecond().indexOf("?") == -1)
    // GenericConverter.createGoldSameAsMLN(goldPair.getSecond(), nellObjPFxd,
    // goldEvidenceWriter);
    // }
    // }
     ...
     * @throws IOException
     */
    public List<String> createEvidenceForTopKCandidates(BufferedWriter allEvidenceWriterTop1,
            BufferedWriter allEvidenceWriter, String nellInst,
            String nellPostFixdInst, Set<String> termConceptPairSet, String identifier)
            throws IOException {
        ...
        sameAsConfidences = DBWrapper.fetchTopKLinksWikiPrepProb(Utilities.cleanse(nellInst)
            ...
            if (!termConceptPairSet.contains(nellPostFixdInst + val)) {
                ...
                allEvidenceWriter
                        .write("sameAsConf("
                                ...
                                + "\", "
                                + decimalFormatter
                                        .format(Utilities.convertProbabilityToWeight(Double
                                                .parseDouble(val
                                                        .split("\t")[1])))
                                + ")\n");
        ...
        if (sameAsConfidences.size() > 0) {
            ...
            conc = Utilities.utf8ToCharacter(sameAsConfidences.get(0).split("\t")[0]);
            conc = MLNFileGenerator.removeTags("DBP#resource/"
                    + Utilities.characterToUTF8(conc.replaceAll("~", "%")));

            allEvidenceWriterTop1
                    .write("sameAsConf("
                            + conc
                            + ", \"NELL#Instance/"
                            + nellPostFixdInst
    ...
    private void generateDBPediaTypeMLN(String dbPediaInstance,
        ...
        try {
            ...
            System.err.println("Exception in generateDBPediaTypeMLN() " + e.getMessage());
        }
    }
    ...
    private void loadGoldStandard() throws IOException {
        BufferedReader tupleReader = new BufferedReader(new FileReader(Constants.INPUT_CSV_FILE));
        String line;
        String key = null;
        ...
        while ((line = tupleReader.readLine()) != null) {
            ...
            valuePair = new Pair<String, String>(line.split("\t")[5], line.split("\t")[6]);

            GS_PAIRS.put(key, valuePair);
        }

        System.out.println("Loaded Gold Standard MAP size = " + GS_PAIRS.size());
    }
}
