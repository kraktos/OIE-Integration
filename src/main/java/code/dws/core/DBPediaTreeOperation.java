...
public class DBPediaTreeOperation {
    ...
    /**
     * @param uNTYPEDNODE the uNTYPEDNODE to set
     */
    ...
    /**
     * @param dbPediaTypes types of the entity, used for querying
     * @param fullSetOfTpes
     * @return a ranked list of DBpedia types for that entity
     * @throws IOException
     */

    public Map<String, Pair<Double, Double>> getNodeScore(List<String> dbPediaTypes,
            Set<String> fullSetOfTpes) {
        ...
        for (Map.Entry<String, GenericTreeNode> entry : COLLECTION_NODES.entrySet()) {
            ...
            rankedNodesMap.put(node.getNodeName().getIRI().toURI()
                    .toString().replaceAll(",", "_"),
                    node.getNodeDownScore());
        }
...
//                if (COLLECTION_NODES.containsKey(Constants.ONTOLOGY_NAMESPACE + type)) {
//
//                    // put the nodes with computed scores in a collection
//                    rankedNodesMap.put(COLLECTION_NODES.get(Constants.ONTOLOGY_NAMESPACE + type)
//                            .getNodeName().getIRI().toURI()
//                            .toString(),
//                            COLLECTION_NODES.get(Constants.ONTOLOGY_NAMESPACE + type)
//                                    .getNodeDownScore());
...
//        }
    ...
    static Map<String, Pair<Double, Double>> sortByValue(Map map, double totalScore) {
        ...
        for (Iterator it = list.iterator(); it.hasNext();) {
            ...
            result.put(entry.getKey(), new Pair<Double, Double>((Double) entry.getValue(),
                    (Double) entry.getValue() / totalScore));
        }
        return result;
    }
    ...
    private void createFullTreeStructure() {
        ...
        double nodeValue = findDomainRangeValueByName(this.UNTYPED.getIRI().toURI().toString()
            ...
            for (GenericTreeNode child : node.getChildren()) {
                ...
                if (!THING.equals(child.getNodeName())) {
                    if (isDirectSubClsOfThing(child.getNodeName())) {
                        ...
                        child.setNodeDownScore(getParentsDownScore(node) + downScore(child));
                    }
                }
                ...
                COLLECTION_NODES.put(child.getNodeName().getIRI().toURI().toString(), child);
                queue.add(child);
            }
        }
    }
     ...
     * @return
     */
    public GenericTreeNode searchForNode(GenericTreeNode tree, OWLClass searchnode) {
    ...
    private double getParentsDownScore(GenericTreeNode node) {
        ...
        try {
            ...
            System.out.println("Exception with " + node.getNodeName() + " \t " + e.getMessage());
            return 0;
        }
    }
    ...
    private OWLClass parentOfNode(GenericTreeNode node) {
        ...
        Set<OWLClass> set = reasoner.getSuperClasses(node.getNodeName(), true).getFlattened();
        for (OWLClass clas : set) {
            if (clas.getIRI().toURI().toString().indexOf(Constants.ONTOLOGY_NAMESPACE) != -1) {
                parent.add(clas);
            }
        }
        return parent.get(0);
    ...
    private double downScore(GenericTreeNode node) {
        return (1 - Constants.TREE_PROPAGATION_FACTOR) * node.getNodeUpScore();
    }
    ...
    private double upScore(GenericTreeNode node) {
        ...
        return node.getNodeValue() + Constants.TREE_PROPAGATION_FACTOR * sumChildScores;
    }
     ...
     */
    private void populateChildren(GenericTreeNode node, List<OWLClass> listChildClasses) {
        ...
        for (OWLClass childClass : listChildClasses) {
            ...
            if (childClass.getIRI().toURI().toString().indexOf(Constants.ONTOLOGY_NAMESPACE) != -1) {

                nodeValue = findDomainRangeValueByName(childClass.getIRI().toURI().toString()
                        .replaceAll(Constants.ONTOLOGY_NAMESPACE, ""));
    ...
    private boolean isDirectSubClsOfThing(OWLClass owlClass) {
        Set<OWLClass> superConcepts = reasoner.getSuperClasses(owlClass, true).getFlattened();
    ...
    private double findDomainRangeValueByName(String topClass) {
        ...
        if (this.getIdentifier().equals(Constants.DOMAIN)) {
            for (Pair<String, Double> pairDomValue : domPairs) {
                if (pairDomValue.getFirst().equals(topClass)) {
                    return pairDomValue.getSecond();
                }
            }
        } else {
            for (Pair<String, Double> pairRanValue : ranPairs) {
                if (pairRanValue.getFirst().equals(topClass)) {
                    return pairRanValue.getSecond();
                }
            }
        }
        return 0;
