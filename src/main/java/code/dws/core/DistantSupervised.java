...
public class DistantSupervised {
    ...
    public void learnDomRanFromFullData() throws IOException {
        ...
        if (Constants.ENGAGE_INTER_STEP) {
            ...
            BootStrapMethod
                    .createSubObjLists(Constants.BASIC_REASON_OUT_FILE);
     ...
     * @throws IOException
     */
    public void processTripleFromOutputFiles()
            throws IOException {
     ...
     * @throws IOException
     */
    private void bootStrap(OIE oieType)
            throws IOException {
        ...
        while ((triple = input.readLine()) != null) {
            ...
            if (this.propertyName != null && this.propertyName.length() > 0)
                if (this.propertyName.equals(arrStr[1])) {
                    ...
                    // process them
                    this.processEachTriple(arrStr[0], arrStr[1], arrStr[2], oieType);
                }
        }
     ...
     */
    public void processEachTriple(String sub, String prop, String obj, OIE oieType) {
    ...
    public void getTypes(List<String> entities, String domRanType) {
        ...
        for (String dbpEntity : entities) {
            ...
            if (Constants.RELOAD_DBPEDIA_TYPES) {
                ...
                entityTypes = DBWrapper.getDBPInstanceType(Utilities.characterToUTF8(dbpEntity));
            }

            // if no entity types, add UNTYPED
            if (entityTypes.size() == 0 || entityTypes.get(0).indexOf(Constants.UNTYPED) != -1) {
     ...
     * @param objEntities list of object candidates
     */
    private void populateDomainAndRangeMaps(List<String> subEntities, List<String> objEntities) {
        List<String> entityTypes = null;
    ...
    public List<Pair<String, Double>> getTopDomainClass(int topK) {
        ...
        for (Map.Entry<String, Double> entry : domainClassMap.entrySet()) {
            if (topK != 0 && listTopKClasses.size() == topK)
                ...
                listTopKClasses.add(new Pair<String, Double>(entry.getKey(), entry.getValue()));
            }
        }
