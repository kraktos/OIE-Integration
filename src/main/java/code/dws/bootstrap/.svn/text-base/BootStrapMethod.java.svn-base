...
public class BootStrapMethod {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // loadOldAlphaNodes(Constants.DOMAIN_RANGE_PREFERENCE_FILE);
        ...
        nodeScorer.rankTypes(GenericConverter.SUB_SET_TYPES, SET_FULL_SUB_TYPES,
                domRangPrefWriter,
        ...
        nodeScorer.rankTypes(GenericConverter.OBJ_SET_TYPES, SET_FULL_OBJ_TYPES,
                domRangPrefWriter,
    ...
    private static void loadOldAlphaNodes(String domainRangePreferenceFile)
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (line.indexOf(Constants.ONTOLOGY_NAMESPACE) != -1 && !flag) {
                SET_FULL_SUB_TYPES.add(line.split("\t")[0].replaceAll(Constants.ONTOLOGY_NAMESPACE,
                        ""));
            }

            if (line.indexOf(Constants.ONTOLOGY_NAMESPACE) != -1 && flag) {
                SET_FULL_OBJ_TYPES.add(line.split("\t")[0].replaceAll(Constants.ONTOLOGY_NAMESPACE,
                        ""));
            }
        }
    ...
    private static void createSameAsConf(String outputEvidenceFilePath) throws IOException {
        ...
        // read the file contents
        BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(
                outputEvidenceFilePath)));

        while ((line = reader.readLine()) != null) {
            m = pattern.matcher(line);
            while (m.find()) {
                SAME_AS_CONF_MAP.put(m.group(1).split(",")[0] + m.group(1).split(",")[1],
    ...
    public static void createSameAsHypotheses(String outputEvidenceFilePath)
        ...
        while ((line = reader.readLine()) != null) {
            ...
            while (m.find()) {
                ...
                SAME_AS_LIST.add(new Pair<String, String>(m.group(1).split(",")[0], m.group(1)
                        .split(",")[1]));
            }
        }
    }
