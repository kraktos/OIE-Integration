...
public class ReverbPreProcessing {
	...
	public static void readTriples(String filePath) throws IOException {
		...
		try {
			...
			Scanner scan;
			scan = new Scanner(
					ReverbPreProcessing.class.getResourceAsStream(filePath),
					"UTF-8");
			...
			while (scan.hasNextLine()) {
				...
				outputWriter.write(simScoreSubj
						* simScoreObj
						+ "\t"
						+ Utilities.convertProbabilityToWeight(simScoreSubj
								* simScoreObj) + "\t" + subType + "\t"
						+ revSubj + "\t" + revProp + "\t" + revObj + "\t"
						+ objType + "\n");
				outputWriter.flush();
	...
	private static void readPropFiles(String filePath) throws IOException {
		...
		ArrayList<ArrayList<String>> reverbPropFile = FileUtil
				.genericFileReader(
						ReverbPreProcessing.class.getResourceAsStream(filePath),
