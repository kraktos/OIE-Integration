...
public class MLNCreator {
	 ...
	 * @throws IOException
	 */
	private static void getPropSpecificData(String prop, long nProp,
			BufferedWriter evidenceWriter) throws IOException {
		String dom = null;
		String ran = null;
		...
		if (subTypesWithCounts.size() > 0)
			for (String result : subTypesWithCounts) {
				...
				System.out.println("P(" + dom + "|" + prop + ")  ="
						+ (double) nDomProb / nProp);

				evidenceWriter.write("domConf(\"DBP#ontology/" + dom + "\", \""
						+ prop + "\", "
						+ Constants.formatter.format((double) nDomProb / nProp)
						+ ")\n");
			}
		...
		if (objTypesWithCounts.size() > 0)
			for (String result : objTypesWithCounts) {
				...
				System.out.println("P(" + ran + "|" + prop + ")  ="
						+ (double) nRanProb / nProp);

				evidenceWriter.write("ranConf(\"DBP#ontology/" + ran + "\", \""
						+ prop + "\", "
						+ Constants.formatter.format((double) nRanProb / nProp)
						+ ")\n");
			}
	...
	private static void createGroundTruths(String weightedAssertionFile)
		...
		while (scan.hasNextLine()) {
			...
			evidenceWriter.write("propAsstConf(\"" + property
					+ "\",\"DBP#ontology/" + domain + "\",\"DBP#ontology/"
