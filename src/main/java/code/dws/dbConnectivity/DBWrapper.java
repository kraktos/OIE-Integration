...
public class DBWrapper {
	...
	public static void initNellReasoner() {
		...
		try {
			...
			logger.error("Connection Failed for batchInit! Check output console"
					+ ex.getMessage());
		}
	}
	...
	public static void init(String sql) {
		try {
			...
			// for reverb
			insertPrepstmnt = connection
					.prepareStatement(Constants.INSERT_GOLD_STANDARD_REVERB);

			insertReverbPrepstmnt = connection
					.prepareStatement(Constants.INSERT_REVERB_TYPE_WEIGHTS_SQL);
	 ...
	 *            apriori probability
	 */
	public static void saveAxiomsPrior(OWLNamedIndividual ieValue,
			OWLNamedIndividual dbpValue, double score) {
		...
		try {
			...
			logger.error(" exception while inserting axioms before inference.."
					+ e.getMessage());
		}
	}
	...
	public static void saveAxiomsPosterior(String valueFromExtractionEngine,
		...
		try {
			...
			logger.error(" exception while inserting axioms after inference.."
					+ e.getMessage());
		}
	...
	public static void saveReverbTypeWeights(double subSim, double objSim,
			String subType, String sub, String prop, String stemProp,
			String obj, String objType) {
		...
		try {
			...
			logger.error("Error with batch insertion of Reverb types .."
					+ e.getMessage());
		}
	...
	public static void saveBaseLine(String ieArg1, String ieRel, String ieArg2,
		...
		try {
			...
			logger.debug("[" + dbPediaTriple.getSurfaceSubj() + ", "
					...
					+ dbPediaTriple.getSurfaceObj() + "]  =>  [" + ieArg1
					+ ", " + ieRel + ", " + ieArg2 + "] ");

			insertBaseLine.setString(1, ieArg1);
			insertBaseLine.setString(2, ieRel);
			insertBaseLine.setString(3, ieArg2);
			insertBaseLine.setString(4,
					Utilities.utf8ToCharacter(dbPediaTriple.getSurfaceSubj()));
			insertBaseLine.setString(5,
					Utilities.utf8ToCharacter(dbPediaTriple.getRelationship()));
			insertBaseLine.setString(6,
					Utilities.utf8ToCharacter(dbPediaTriple.getSurfaceObj()));
			...
			logger.error("Error with batch insertion of base lines .."
					+ e.getMessage());
		}
	...
	public static void saveGoldStandard(FreeFormFactDao nellTriple,
		...
		try {
			...
			fetchCountsPrepstmnt.setString(
					2,
					Utilities.cleanse(nellTriple.getSurfaceSubj()).replaceAll(
							"_", " "));
			...
			fetchCountsPrepstmnt.setString(
					2,
					Utilities.cleanse(nellTriple.getSurfaceObj()).replaceAll(
							"_", " "));
			...
			logger.debug("[" + nellTriple.getSurfaceSubj() + ", "
					+ nellTriple.getRelationship() + ", "
					+ nellTriple.getSurfaceObj() + "]  =>  [" + arg1 + ", "
					+ rel + ", " + arg2 + "] " + subjLinksCount + " "
					+ objLinksCount);
			...
			logger.error("Error with batch insertion of gold standards.."
					+ e.getMessage());
		}
	}
	...
	public static List<Double> fetchNELLConfidence(String sub, String pred,
			String obj) {
		ResultSet rs = null;
		...
		try {
			...
			logger.error(" exception while fetching fetchNELLConfidence " + " "
					+ e.getMessage());
		}
	...
	public static Set<String> getAllSurfaceForms(Set<String> aLL_SURFACES,
			String arg) {
	...
	public static List<FreeFormFactDao> getTriples(
		...
		try {
			...
			while (rs.next()) {
				allDupliTriples.add(new FreeFormFactDao(rs.getString(1), rs
	...
	public static void saveSurfaceForms(String uri, String sf, double sfGivenUri) {
		...
		try {
			...
			logger.error("Error with batch insertion of surfaceForms .."
					+ e.getMessage());
		}
	...
	public static List<FreeFormFactDao> giveDupliRows(FreeFormFactDao nellTriple) {
		...
		try {
			...
			while (rs.next()) {
				retList.add(new FreeFormFactDao(rs.getString(1), rs
						.getString(2), rs.getString(3)));
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static List<String> getTopTriples(FreeFormFactDao ieTriple) {
		...
		try {
			...
			while (rs.next()) {
				results.add(rs.getString(1) + GS_DELIMITER + rs.getString(2)
						+ GS_DELIMITER + rs.getString(3) + GS_DELIMITER
						+ rs.getString(4) + GS_DELIMITER + rs.getString(5)
						+ GS_DELIMITER + rs.getString(6));
			}
		} catch (SQLException e) {
	...
	public static List<String> getGoldTriples(
			List<String> allDistinctGoldTriples) {
		try {
			ResultSet rs = pstmt.executeQuery();

			while (rs.next()) {
				allDistinctGoldTriples.add(rs.getString(1) + GS_DELIMITER
						+ rs.getString(2) + GS_DELIMITER + rs.getString(3)
						+ GS_DELIMITER + rs.getString(4) + GS_DELIMITER
						+ rs.getString(5) + GS_DELIMITER + rs.getString(6));
			}
	...
	public static void insertIntoPredTypes(String nellPredicate,
			List<String> listTypes, String instance) {
		try {
	...
	public static List<ImmutablePair<String, String>> getReverbInstances(
			String propArg) {
